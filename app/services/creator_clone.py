...
- summary 必须是 2-4 句高密度中文，直接回答“这个账号主要怎么做内容，下一条最值得尝试什么”。
...
- summary 必须直接告诉用户“这个账号的具体做法、下一条值得借鉴什么结构”。
...
- 输出要可执行，不要只写摘要。优先给少数有信息量的结论，说明“为什么值得尝试 / 适用场景 / 风险边界”，不凑条数。
...
- 对美拍/COS/摄影出片类账号，拍摄动作公式要包含首帧、人物动作、镜头或光线、标题话题、验证指标和风险边界；没有观察依据的部分作为未来尝试，不冒充原视频事实。
- transferable_formulas、candidate_ideas 和 creator_clone_spec.self_check_rubric 优先保留少数有信息量的内容，不为凑数量补通用模板。
...
- 每个可执行模块优先给少数高密度结论，不凑条数；观察依据来自分层、证据矩阵或 batch 摘要，解释与未来建议要说明各自性质。
...
def content_profile_prompt_text(sample_set: CloneSampleSet, selected_samples: list[CloneSample]) -> str:
    ...
    return (
        ...
        "核心判断表达要求（复用现有字段，不增加字段）：\n"
        "- creator_positioning 和 creator_clone_strategy.positioning 要概括具体定位，不止返回内容大类："
        "结合主要内容对象、呈现方式和有辨识度的做法，用一句易记的话说明账号核心；观众吸引力是解释或假设，不是已验证受众事实。\n"
        "- summary 保持 2-4 句高密度表达，先给判断：主要怎么做内容、本轮最值得关注的一两个做法、下一条最值得尝试什么。"
        "定位标题概括账号，摘要展开方法、简短依据与建议；允许共享关键词，但不要整句无增量复述。不要以分类或检查流程开场。\n"
        "- 特征要落到材料中的题材、呈现、动作、组织或标题做法；使用“情绪价值”“身份共鸣”等术语时，紧接着解释具体怎么做。"
        "按当前内容方向分析，教程讲步骤与演示，知识讲观点与例证，剧情讲行动与揭示，不统一套成摄影模板。\n"
        "- 按观察、解释、执行组织：可以基于材料提出有判断力的解释与创意，不必都已实验证明；"
        "但没有提供画面不声称亲眼看过，没有有效转录不编台词，互动排名不证明因果或完播提升。"
        "缺口只限制相关判断，用简短限定或 evidence_gaps 说明，不把整份分析退化成无法确认。\n"
        "- transferable_formulas、candidate_ideas、next_actions 说明保留哪个结构、替换什么题材或场景、下一条具体做什么及为什么值得试；"
        "不要只说提升节奏、增强氛围。新创意明确是建议，不能写成原作品发生的事实；少数具体方法优于重复的通用建议。"
    )
