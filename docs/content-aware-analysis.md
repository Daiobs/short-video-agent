...
本轮沿用同一次成功模型输出，**没有重新生成**。A/B/C 原文件保持不变；C-review 仅在私有隔离副本重做正常化、派生说明与展示，人工审阅注释单独标识。用户产品验收仍 PENDING。

### 引用与来源合同

- 真实问题位于 `expression_patterns.subtitle_voice[0].evidence`：不存在的样本引用此前未被 focused-analysis 校验覆盖，后续还进入派生 hooks。
- 小型共享解析器覆盖已有报告根节点内的 `evidence / evidence_refs / references / sample_ids / supporting_samples`，包括标量、列表、嵌套对象及 JSON 编码引用；`sample_id / case_id / aweme_id` 仅使用服务端明确映射。普通业务 `id`、任意数字、台词和 URL 不递归当作引用删除。
- 新结果以实际成功请求与选中样本的交集解析；历史缺失请求记录时，`reference_manifest.scope=historical_unknown`、`identity_scope=selected_only`，只说明身份可定位，不宣称本次提交过。无模糊纠错、无模型标题认领身份。
- 无效支持移除，保留条目和可定位、幂等的警告；叙述中的完整样本 ID 保留原文以供审计，展示时标记不可定位。保存、重读不重复报警、不恢复无效支持。
- 合法引用展示服务端标题/友好编号与已有安全 Case 入口；仅样本级关联明确提示“未提供具体片段定位”，不等同事实验证。没有引用才称未单列支持依据。
- `request_evidence` 与归档库存独立：C 实际两张图覆盖两条样本、OCR 五条、ASR 两条、有效评论零条、三条没有直接图片。保留库存中的评论记录，不倒推为本次模型读过。
- 兼容读取重建派生 view model；详情 API 和已有 JSON/Markdown/HTML 导出使用相同适配，不批量重写原文件。保留已有 HTML 缺失时的懒生成行为，其他缺失文件仍返回原有错误。

| 内容字段 | 阅读位置与保留方式 |
| --- | --- |
| `expression_patterns` 原始对象 | 核心规律优先使用结构化条目，避免压平副本泄漏枚举；完整条目可展开 |
| 公式、选题、行动与 focused findings | 默认核心内容、已有完整细节与统一参考样本；不编造片段定位 |
| `reference_manifest` 无效项 | 对应条目提示需复核，原始记录放详情 |
| 归档计数 / 成功请求材料 | 分别标为库存和本次输入；没有记录时明确未知 |
| 模型自评 / 结构分 | 次要说明，不称“高可信”或事实准确率 |

### 内容限制与手机阅读

OCR 与 ASR 存在文字差异，C 原模型却称二者一致；C-review 分列两个识别来源，未核验原音，不自动判定 OCR 正确。原“固定机位”“始终居中”等说法保留，独立注释说明静态 contact sheet 无法确认整段运动、连续剪辑或音乐卡点。下一条拍摄建议与原作观察分开。未来 Prompt 只补充简短来源冲突与静态观察边界，尚未用新模型调用验证其效果。

手机在短摘要后增加行动跳转入口，再读核心规律和完整行动；产品内跳转不改变 `#profile`。桌面仍是规律与行动并排。五条样本保持五张多指标卡；完整内容、重要限制和技术详情仍保留。验收头部高度单独记录，不作为产品首屏的内容量。

### 保存响应的隔离网页回放

在独立临时数据库、素材副本及独立进程中，使用正式网页按钮、原 Job 路由/执行、Provider JSON 处理、正常化/保存及轮询。HTTPX 传输边界返回已保存的完整 Responses JSON；未知 HTTP 和非 loopback socket 出站直接拒绝，使用无效测试 Key，不读取真实凭据。回放装置不进入生产路由，也不真实等待原来的 203 秒。

验证首次报告可见、刷新恢复，以及模拟 401 后旧报告字节保持不变、仍可查看。失败沿用现有任务层的降级完成表达，不能把该状态当作新的模型成功。回放是本地交互验收，**不是第四次真实模型生成**。

### 输出 token 记录

保存的出站断言、请求哈希与配置记录确认 `max_output_tokens=1800` 已发送；保存响应 `completed`，`output_tokens=6244`，其中 `reasoning_tokens=644`，响应 `max_output_tokens=null`。不能将差额全部解释为思考 token。网关是否执行该上限未确认，1800 不是已验证的成本上限；本轮不调查网关、不发探针、不改输出设置。

验证：完整 `pytest -q` **1145 passed、1 个既有弃用警告**；12 个 JavaScript 文件语法、`compileall`、`git diff --check` 通过。旧库存等同输入断言改为分别核对库存与未知输入；原 Prompt 主体长度检查仍保留，新简短约束独立限长；没有删除既有有效回归。

Chrome 独立上下文 100% 缩放，1280px / 390px：默认与全部展开均无水平溢出，无页面异常，五张合并指标样本卡；手机摘要后有可用行动入口，实际网页点击后 hash 仍为 `#profile`，安全 Case 入口返回200。C-review 的独立验收头部单独记录，不能用原 C 不同头部高度声称产品整体提速。

最终网页回放创建两个隔离 Job：保存的成功响应、模拟401；均走正式任务/轮询路径。首次可见、刷新恢复、失败后旧结果字节保留与重新查看均通过，无页面异常。调试回放曾误等待已折叠的完整摘要，修正为检查可见摘要与真实保存结果后通过；它不是业务“必须刷新”故障，更不是一次新的真实调用。

260 个受保护原文件修改0，真实配置与 A/B/C 哈希一致，8765进程未重启。原始报告、完整响应、Prompt、截图及回放脚本只保留在私有隔离目录，不进入 Git。新增真实模型调用0，历史累计任务级生成3。PR #29保持Draft，不Ready、不Merge、不部署；用户产品验收PENDING。

`STOPPED_AFTER_PR29_REAL_REPORT_REFERENCE_AND_UX_CLOSEOUT`
