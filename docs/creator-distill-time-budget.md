...
| --- | ---: |
| 普通分析单次请求 | 90 秒 |
| Creator 蒸馏单次请求 | 180 秒 |
| quick | 240 秒 |
| deep | 600 秒 |
| Batch Job | 600 秒 |
| Final Reduce 单次请求 | 600 秒 |
| Final Reduce 最低预留 | 120 秒 |
| 精简重试最低剩余 | 60 秒 |

普通分析和 Creator 蒸馏使用独立的单请求上限。每次 Creator 请求实际 timeout 为：

```text
min(Creator 单请求上限, 当前剩余总预算)
```

总预算是从蒸馏开始计时的硬墙钟上限，不是保证等待时长。Creator 页面通过“响应模式”显式选择 Quick 或 Deep。Quick 默认最多等待 240 秒，遇到 timeout 立即停止并提示切换 Deep；只有无效 JSON 或 502/503 等明确可恢复错误，才可能在剩余预算充足时精简重试一次。Deep 默认总预算 600 秒，允许 timeout、无效 JSON 或 502/503 在剩余预算不少于 60 秒时精简重试一次。Prompt 长度、样本数和已知视频时长用于诊断和批次规划，不自动提高总预算。
